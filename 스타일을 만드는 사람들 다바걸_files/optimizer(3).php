...
/**
 * EC내부에서 사용하는 JS를 관리하는 스크립트
 * 이스크립트를 사용할 경우 페이지의 속도가 떨어지는 문제를 해결 할 수 있습니다.
 * 
 * 해당 몰에 사용할 메소드를 찾으신 후 제일 아래줄에 추가해주면 됩니다.
 */
...
EC$(function() {
     ...
     * ACE COUNT
     */
    if (aExternalJs['ace_count'] === 'y') {
        EC_EXTERNAL_JS.aceCounter(aExternalJs['ace_count_id']);
    }
    
    /**
     * asp3
     */
    if (aExternalJs['asp3'] === 'y') {
        EC_EXTERNAL_JS.asp3(aExternalJs['asp3_name']);        
    }
    
    
    if (aExternalJs['adnaver'] === 'y') {
        EC_EXTERNAL_JS.asp3(aExternalJs['adnaver_key'], aExternalJs['adnaver_sidx'], aExternalJs['adnaver_ua']);        
    }
    
    
    if (aExternalJs['adcounter'] === 'y') {
        EC_EXTERNAL_JS.adcount(aExternalJs['adcounter_key'], aExternalJs['adcounter_sidx'], aExternalJs['adcounter_ua']);
    }
    
    
    if (aExternalJs['nasmedia'] === 'y') {
        EC_EXTERNAL_JS.nasmedia(aExternalJs['nasmedia_camp'], aExternalJs['nasmedia_page'], true);
    }
...
var EC_EXTERNAL_JS = 
{
     ...
     * ACE COUNTER
     * 에이스 카운터의 외부 스크립트  
     * @param    string   에이스 카운터 ID
     * @example   EC_EXTERNAL_JS.aceCount("1C518304J1P5GJK3ELIBBRVNRRK");
     */ 
    aceCounter: function (sYsmAccountId)
    {
        window.ysm_customData = new Object();
        window.ysm_customData.conversion = "transId=,currency=,amount=";
        var sSrc = '//srv2.wa.marketingsolutions.yahoo.com/script/ScriptServlet?aid='+sYsmAccountId;
        this.createScript(sSrc);
    },

    /**
     * asp3.http.or.kr
     * asp3 이 사이트는 그대로 사용하면 될 듯한디요.. 자체적으로 지들이 관리하고 있고
     * noscript에 대한 대응도 하고 있어서 그래도 사용하길 추천함.
     * @param       string  회원명
     * @example     EC_EXTERNAL_JS.asp3('회원명');
     */
    asp3: function(sMemberName)
    {
        http_msn_member_name = sMemberName;        
        //var sSrc = 'http://asp10.http.or.kr/http_msn/usrconfig/gtattoo/js/asp_conf.js';
        //this.createScript(sSrc);
    },
     ...
     * adnaver.wsos.co.kr
     *
     * @param   string   sKey    ih_key
     * @param   int      iIdx    ih_sidx
     * @param   int      ihUa    ih_ua
     * @example  EC_EXTERNAL_JS.adnaver(발급키, 인덱스, 1);
     */
    adnaver: function(sKey, iIdx, ihUa) 
    {
        iIdx = iIdx || 631;     
        ihUa = ihUa || 1;
        var sSrc='http://analysis.wsos.co.kr/analysis/analysis.php?_ih_sidx='+iIdx+'&_ih_ua='+ihUa+'&_ih_key='+sKey;
        this.createScript(sSrc);
    },
     ...
     * @param   int      iIdx    ih_sidx
     * @param   int      ihUa    ih_ua
     * @example  EC_EXTERNAL_JS.adcount(발급키, 인덱스, 1);
     */ 
    adcount: function(sKey, iIdx, ihUa) 
    {
        iIdx = iIdx || 1124;        
        ihUa = ihUa || 1;
        var sSrc = 'http://adcounter.emnet.co.kr/analysis/analysis.js?_ih_sidx='+iIdx+'&_ih_ua='+ihUa+'&_ih_key='+sKey;
        this.createScript(sSrc);
    },
    ...
    /**
     * nasmedia.co.kr
     * @param   int   iCamp    NSmart_CAMP
     * @param   int   iPage    NSmart_Page
     * @example  EC_EXTERNAL_JS.nasmedia(iCamp, iPage, true);
     */
    nasmedia: function(iCamp, iPage, bInclude)
    {
        var nasJsHost = (('https:' == document.location.protocol) ? 'https://' : 'http://');
        
        if (bInclude === true) {            
            var sSrc = nasJsHost+'n00.nsmartad.com/etc?id=1';       
            this.createScript(sSrc);            
        }


        if (typeof(NTrackObj) === 'undefined')
        {
            setTimeout('EC_EXTERNAL_JS.nasmedia('+iCamp+', '+iPage+', false)', 200);
            return;
        }

        var NSmart_CAMP = iCamp;
        var NSmart_Page = iPage;
        var NSmart_TURL = nasJsHost + 'n17.nsmartad.com';
        NTrackObj.init(NSmart_CAMP, NSmart_Page, NSmart_TURL);
     ...
     * DOM으로 SCRIPT를 생성함
     */
    createScript: function(sSrc, sId)
    {
        var node = document.createElement('script');
        node.setAttribute('type', 'text/javascript');
        node.setAttribute('src', sSrc);
        if (sId) {
...
var CAPP_ASYNC_METHODS = {
    ...
    init: function()
    {
        ...
        if (aNoCachedModules.length > 0) {
            ...
            EC$.ajax(
            {
                url: '/exec/front/manage/async?module=' + aNoCachedModules.join(',') + sEditor + sPathRole + sEcMobile,
                dataType: 'json',
                success: function(aData)
                {
